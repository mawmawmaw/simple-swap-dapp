...
async function getAvailableTokens(userSelectedChain) {
    ...
    const availableTokens = await dex.getSupportedTokens({
...
async function getBalance(token) {
    console.log("Get balance from token: "+token);
    const options = {
...
async function getQuote() {
    if(swapFromTokenAddress !== '' && swapToTokenAddress !== '' && document.querySelector("#swap-from-amount").value > 0){
        amountChosenToSwap = Number(Moralis.Units.ETH(document.querySelector("#swap-from-amount").value));
        ...
        if(swapQuote.error){
            ...
            document.querySelector("#swap-to-amount").value = Number(Moralis.Units.ETH(swapQuote.toTokenAmount));
  ...
  async function swap() {
    const receipt = await dex.swap({
      ...
      fromTokenAddress: swapFromTokenAddress, // The token you want to swap
      toTokenAddress: swapToTokenAddress, // The token you want to receive
      amount: Number(Moralis.Units.ETH(amountChosenToSwap)),
      fromAddress: userAddress, // Your wallet address
      slippage: 1,
    });
    console.log(receipt);
